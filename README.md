...
and every constant `c ∈ R`, closed under `(f, g) ↦ p(f, g)`; call `p`
*complete* if `Clo p = R[x,y]`.

**Precision, for universal algebraists.** In clone-theoretic terms:
`Clo p` is the binary part of the subclone of the *polynomial clone* of
`R` (equivalently, of the clone of the Lawvere theory of commutative
`R`-algebras — constants included as nullary operations) generated by
`p`. Completeness asks whether it reaches the polynomial clone
generated by `{x+y, x·y}`, i.e. all of `R[x,y]`. **This is NOT a claim
about the full clone of all finitary operations on the set `R`**, which
is larger for finite `R` and wildly different for infinite `R`.
Moreover the development works with polynomials as *formal* objects:
over an infinite integral domain this coincides with polynomial
*operations*, but over a finite field it is strictly finer. The two
readings can disagree on the same operation: `1 + xy` induces NAND on
`F₂`, which is functionally complete, yet formally `x + y ∉ Clo(1+xy)`
— our F₂ theorem is invisible to, and does not contradict, classical
functional completeness theory (Post; Webb; Rosenberg). Conversely,
for odd prime powers `q`, completeness of `x² − y` over `F_q` plus
Lagrange interpolation implies that `⟨F_q; x²−y⟩` *with constants* is
functionally (polynomially) complete. Primality of `⟨F_q; x²−y⟩`
