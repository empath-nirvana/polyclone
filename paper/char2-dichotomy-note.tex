...
\begin{abstract}
Say that $p \in R[x,y]$ \emph{generates} $R[x,y]$ if every bivariate
polynomial over $R$ is an iterated composition of $p$, the variables,
and constants.  We prove: there is a single polynomial generating all
of $R[x,y]$ under composition if and only if $2$ is a unit of $R$;
when it is, $x^2 - y$ is a generator, and when it is not, for every
$p$ at least one of $x+y$, $xy$ is inexpressible.
\end{abstract}
...
$\Clo(p) \subseteq R[x,y]$ is the smallest set containing $x$, $y$,
and every constant $c \in R$, and closed under
$(f,g) \mapsto p(f,g)$ (substitution of $f$ for $x$ and $g$ for $y$
in $p$).  Say $p$ \emph{generates} $R[x,y]$ if $\Clo(p) = R[x,y]$.
\end{definition}

In clone-theoretic terms: the binary terms in the language of
commutative rings with constant symbols form the binary part of an
abstract clone, canonically identified with $R[x,y]$ under
substitution, and our question asks when the single term $p$
generates it.  This abstract object is represented faithfully by
operations on $R$ when $R$ is an infinite integral domain, and is
finer than its image otherwise; it should not be confused with the
clone of polynomial operations on $R$ in the sense of universal
algebra, nor with the full clone of all finitary operations on $R$.
The distinction between formal polynomials and the functions they
induce matters over finite fields, and the contrast is sharpest
there: by a theorem of Webb \cite{Webb}, a single binary operation
generates every operation on any finite set --- in particular every
polynomial function over $\F_2$ is a composition of a single binary
polynomial function --- while Theorem~\ref{thm:F2} shows that no
single polynomial generates $\F_2[x,y]$ formally.  The formal
statement therefore has no functional counterpart.  Concretely,
$n := 1 + xy \in \F_2[x,y]$ induces the binary NAND operation on
$\F_2$ (the Sheffer stroke \cite{Sheffer}), and every Boolean
function is a composition of NAND; nevertheless
$x + y \notin \Clo(n)$ by Theorem~\ref{thm:F2}, since $Dn = 1 \neq 0$
for the operator $D$ of Section~\ref{sec:negative}.
...
If $\tfrac12 \in R$ then $q(x,y) := x^2 - y$ generates the full
polynomial ring: $\Clo(x^2 - y) = R[x,y]$.
\end{theorem}
...
No binary polynomial generates $\Z[x,y]$.
\end{corollary}
...
\begin{proof}
Substituting members of $\Clo(p)$ into a member lands in $\Clo(p)$
(induction on the formula).  So if $x+y, xy$ are members, $\Clo(p)$
...
\noindent\emph{Related work.}  Webb's theorem \cite{Webb} settles
the function-setting analogue on every finite set in the positive
direction, in contrast with Theorem~\ref{thm:F2}.  The nearest prior
work on formal polynomials we know of is that of Aichinger and
Kreinecker on generation in $(\Z[x], +, \circ)$
\cite{Aichinger2013,AichingerKreinecker}: univariate, over $\Z$,
with addition retained as a primitive, and including
$2$-divisibility obstructions such as
$2x^{10} \in \langle x^2 \rangle$ but
$x^{10} \notin \langle x^2 \rangle$.  We are not aware of prior work
on the composition-only single-generator question for formal
polynomial rings.
...
All steps below are instances of
...
\emph{Linear span step.}  For $\alpha \in \Clo(q)$ and constant $k$,
...
\emph{Scaled squares.}  The increment step with $\alpha = y^2 + \ell$ (in the
...
We call the gcd extraction \eqref{eq:peel} the \emph{peel}; it
removes the possible curve of critical points of $q$ along
$\{w = 0\}$, and Lemma~\ref{lem:keystone} shows none survives in
$h$.  Three lemmas, then the descent.
...
\begin{lemma}[coprimality of the peeled partials]\label{lem:keystone}
...
\emph{Derivative identities.}  The chain rule for $h$, with
...
\emph{Case 1: $A$ and $B$ share a root $\theta \in K$.}  Then
...
the derivative identities force $\alpha' = \beta' = 0$, so
...
universally quantified inside, the twist is absorbed.  The parity
decomposition, Proposition~\ref{prop:bridge}, and
Proposition~\ref{prop:master} are unaffected.
\end{proof}
...
If $2$ is a unit, $x^2 - y$ generates $R[x,y]$
(Theorem~\ref{thm:converse}).  If not, $2$ lies in a maximal ideal
$\mathfrak m$; a generator would push forward along
...
Three questions remain open.  Which degrees $\geq 3$ admit single
generators over $\Q$ ($x^3 - y$ is a candidate; over $\F_3$,
$\Clo(x^3 - y)$ consists of additive-plus-constant polynomials, so
$x^3 - y$ does not generate there)?  Which $p$ generate when $2$ is a
unit ($xy + 1$ and $x^2 + y$ do not, by a characteristic-zero
instance of $(\dagger)$ and by a positivity obstruction,
respectively)?  Is membership in $\Clo(p)$, or generation,
decidable?  The identity
...
% Quaest. Math. 44 (2021), no. 5, 693--715, with an author list
% differing from the arXiv version; checked only against search
