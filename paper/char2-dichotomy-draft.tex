...
inexpressible.  More generally, a complete binary polynomial exists
over a commutative ring $R$ if and only if $2$ is invertible in $R$:
when it is, the single operation $p(x,y) = x^2 - y$ already generates
everything; when it is not, completeness fails through a residue
field of characteristic $2$.  The main work is the negative result
...
\subsection{A Sheffer stroke for $\Z[x,y]$?}

Every Boolean function can be built from a single connective: NAND
(the \emph{Sheffer stroke}).  One-element generating sets of this kind
...
Section~\ref{sec:context} explains how this relates to --- and differs
from --- the standard notion of a \emph{clone} in universal algebra,
and no background from that subject is needed).  Call
$p$ \emph{complete} if $\Clo(p) = \Z[x,y]$ --- if the single
operation $p$ generates everything.

Could a complete $p$ exist?  Nothing obvious rules it out; a single
...
$+$ and $\times$, completeness is equivalent to just two
memberships (Lemma~\ref{lem:criterion}):
\[
  p \ \text{complete} \iff x + y \in \Clo(p) \ \text{ and } \
  xy \in \Clo(p).
\]
...
$\Clo(p)$.  In particular, no binary polynomial over $\Z$ is complete.
\end{theorem}

\subsection{Why characteristic two}

Where should such a theorem come from?  The first observation is that
completeness survives reduction of coefficients: a ring homomorphism
$R \to S$ carries any derivation of $x+y$ over $R$ to one over $S$.
Reducing mod $2$, Theorem~\ref{thm:Z} therefore follows from the
corresponding statement over the two-element field $\F_2 = \Z/2$:
...
functionally complete operations on finite sets.  Second --- and this
matters for the reduction --- there is \emph{no hypothesis on $q$
whatsoever}.  Reduction mod $2$ can collapse degree
($2x^3 + xy \mapsto xy$), so to deduce Theorem~\ref{thm:Z} one needs
the $\F_2$-statement for completely arbitrary $q$, including the
low-degree and degenerate ones.

Passing to $\F_2$ is not merely a convenience; characteristic $2$ is
where the obstruction actually lives.  In Section~\ref{sec:converse}
we will see that over any commutative ring in which $2$ is invertible,
the modest operation
\[
  p(x,y) \;=\; x^2 - y
\]
is complete --- a dozen lines of explicit algebra, valid over $\Q$,
$\mathbb{R}$, $\Z[\tfrac12]$, and $\F_p$ for every odd prime $p$.  (Over
$\F_3$, for example, iterating $x^2 - y$ from the variables and
constants produces \emph{every} polynomial in $\F_3[x,y]$.)  The same
derivation, run over $\Z$, fails exactly at the steps that divide by
$2$; Theorem~\ref{thm:Z} says this failure is no accident of one
derivation.  Summarizing both directions:
...
The ``only if'' direction stacks three reductions on top of the
$\F_2$-argument.  First, the descent behind Theorem~\ref{thm:F2}
generalizes to every \emph{perfect} field of characteristic $2$
(one in which every element has a square root --- all finite fields
$\F_{2^k}$ and all algebraically closed fields, for instance), at the
cost of a single twist explained in Remark~\ref{rem:perfect}; the
disjunctive conclusion of Theorem~\ref{thm:F2} holds verbatim there.
Second, an arbitrary field of characteristic $2$ embeds into its
perfection, and non-membership in the clone descends along coefficient
embeddings, so in fact \emph{no field of characteristic $2$} admits a
complete operation.  Third, if $2$ is not a unit of $R$, it lies in a
maximal ideal; completeness pushes forward along the quotient map to
the residue field, which has characteristic $2$.

\subsection{The shape of the proof, in words}\label{sec:shape}
...
operator $D := \partial_x \partial_y$ carries unusual weight.  We
split according to $Dq$.

If $Dq = 0$ --- that is, no monomial of $q$ has both exponents odd ---
then a pleasant chain-rule identity shows the property ``$Dg = 0$'' is
preserved by every substitution into $q$.  The variables and constants
have it; $xy$ does not ($D(xy) = 1$).  So $xy \notin \Clo(q)$, and we
are done with this case: a conserved quantity, nothing more.

If $Dq \neq 0$ we will show $x + y \notin \Clo(q)$, and all the work
lives here.
...
$\{x + y = \text{const}\}$ converts it into the following concrete
...
a constant transcendental over $\F_2$, living in a large
algebraically closed field $K$ of characteristic $2$.  Such a pair
$(\alpha, \beta)$ would be a polynomial-parametrized curve lying
inside a single generic level set of $q$; we call it a
\emph{witness}.  The task is to show witnesses do not exist when
$Dq \neq 0$.
...
fork.  Either some polynomial relation with $\F_2$-coefficients holds
along the witness --- and any such relation is fatal, because it pins
the witness to algebraic data, contradicting the genericity of the
level $c$ --- or no such relation fires, in which case the square
structure consumes the witness itself: both $\alpha$ and $\beta$ turn
...
degree cannot halve forever, so eventually the fatal branch fires.
...
one might call its \emph{Frobenius depth}.  We found this inversion
--- inducting on the counterexample's Frobenius depth rather than on
any complexity measure of $q$ --- to be the key that unlocked the
problem, after many attempts at induction on degree-like measures of
$q$ failed.  Degree of $q$ is a poor induction variable here because
...
presents proofs as ordinary mathematics.

\section{Terminology, and relation to classical completeness theory}
\label{sec:context}

\subsection{On the word ``clone''}\label{sec:cloneword}
...
\cite{Post,Szendrei}.  Our $\Clo(p)$ is not a clone in that sense, and
we should say precisely what it is.  Its elements are formal
polynomials, not maps; the standard home for these is the
\emph{abstract} clone (equivalently, the Lawvere theory
\cite{Lawvere}) of the variety of commutative $R$-algebras, whose
$n$-ary component is exactly the polynomial ring
$R[x_1, \dots, x_n]$, with substitution as composition.  Inside that
abstract clone, $\Clo(p)$ is the binary component of the smallest
subclone containing $p$, the constants, and the projections.
(Restricting to the binary component loses nothing: any term built
from $p$, constants, and two variables is itself a bivariate
polynomial, whatever arities its subterms pass through.)  A reader who
prefers to avoid clones altogether can take the low-tech view of
...
to \emph{completeness with constants}.  (The constant-free variant is
also natural; we do not pursue it here.)  Second, the phrase
\emph{polynomial completeness} has an established and quite different
meaning in universal algebra --- that every congruence-compatible
function on an algebra is induced by a polynomial, as in the
monograph \cite{KaarliPixley} --- and our usage of ``complete'' should
not be confused with it.

\subsection{Functional completeness, and why this question is
different}\label{sec:functional}

For operations-as-functions on a finite set, one-element completeness
...
single complete operation on every finite domain \cite{Webb}; and
there are effective completeness criteria, from S{\l}upecki through
Rousseau to Rosenberg's classification of the maximal function clones
\cite{Slupecki,Rousseau,Rosenberg}.

How does the present question relate?  Over an infinite integral
domain a polynomial is determined by the function it induces, so over
$\Z$ or $\Q$ our statements can equally be read in the function-clone
world (on the infinite set $\Z$ or $\Q$, where the classical
finite-domain theory is silent).  Over finite fields, however, the
formal question is strictly finer than the functional one, and the
two can give \emph{opposite verdicts on the same operation}:

\begin{example}\label{ex:nand}
NAND is a polynomial: $n(x,y) := 1 + xy \in \F_2[x,y]$ induces the
Sheffer stroke on $\{0,1\}$, so the \emph{function} clone it generates
contains every Boolean function --- functional completeness, no
constants needed.  But $D(1+xy) = 1 \neq 0$, so the proof of
Theorem~\ref{thm:F2} shows that the \emph{polynomial} $x + y$ is not a
composition of $n$: every NAND-formula whose induced function is XOR
yields a polynomial that agrees with $x+y$ only as a function on
$\F_2^2$, never on the nose (the difference is a nonzero polynomial
vanishing identically on $\F_2^2$, e.g.\ a multiple of $x^2 + x$).
Thus on $\F_2$, functional completeness is abundant while formal
completeness is impossible: Theorem~\ref{thm:F2} is invisible to
function-clone theory.  In the other direction, over $\F_3$ our
Theorem~\ref{thm:converse} produces every \emph{polynomial}, which is
strictly more than producing every function.
\end{example}

\subsection{Composition structures on polynomial rings}
...
\section{Definitions and first reductions}\label{sec:prelim}
...
$p$.  We call $p$ \emph{complete} over $R$ if $\Clo(p) = R[x,y]$.
\end{definition}
...
members of the clone into a \emph{member} of the clone again lands in
the clone (induction on the formula).  So if $x+y$ and $xy$ are
members, the clone is closed under the derived binary operations
...
homomorphism to $\F_2$ ($\Z/2^k$, $\F_2$-algebras, \dots).  The rest
of the paper, until Section~\ref{sec:converse}, takes place over
$\F_2$.
...
\section{The derivative dichotomy over $\F_2$}\label{sec:dichotomy}
...
above).  Hence $q$ can be written, uniquely, as
...
\subsection{The degenerate case $Dq = 0$: a conserved quantity}
...
\section{A rigidity property of substitution}\label{sec:tameness}
...
already.  The property is engineered to feed the following induction.
...
in $t$, with the convention that constants --- including the zero
polynomial --- have degree $0$; under this convention
$\deg(\alpha_1^2) = 2\deg\alpha_1$ holds without exception, which
keeps the bookkeeping of the descent honest.  Recall the two
...
\]
so $AB$ is a \emph{square} in $K[t]$.  This is the engine: along a
witness, the product of the two partial derivatives is forced to be a
perfect square.

Now the fork.  (For this warm-up we take for granted that neither $A$
nor $B$ vanishes identically; this degeneracy, like several others in
the general case, is dispatched by the kill lemma below ---
Lemma~\ref{lem:kill} --- and we do not belabor it here.)  Suppose
...
easy here, and is Lemmas~\ref{lem:keystone}--\ref{lem:algpoint} in
general) that $P$ must have coordinates \emph{algebraic} over $\F_2$.
But then $c = q(P)$ is algebraic --- contradicting transcendence of
the level.  Generic level sets pass through no critical points; that
is what genericity is for.
...
$\alpha_1, \beta_1 \in K[t]$ of \emph{half} the degree.  And now the
crucial use of $\F_2$-coefficients: squaring fixes $0$ and $1$, so
...
must eventually fire, giving the contradiction.  This proves
...
First, the engine identities \eqref{eq:engine-warm} acquire the factor
...
Second and more seriously, the fork's first branch needs critical
...
consists of critical points, and a witness might conceivably ride
along near it.  The remedy is to peel $w$ off before running the
engine.  Write $A_i = w B_i$, so that the $B_i$ have no common factor,
and
...
\end{equation}
with $B_3 \neq 0$.  We will run the warm-up argument \emph{on $h$
instead of $q$}: the function
$H := h(\alpha,\beta)$ is no longer constant along the witness, but
--- and this is the small miracle that makes the peel legal --- its
derivative still vanishes.  From \eqref{eq:peel} and
$q(\alpha,\beta) = c = r^2$:
...
so $H' = 0$: \emph{the moving level of $h$ is Frobenius-flat}.  The
engine only ever used the vanishing of the level's derivative, so it
runs on $h$ unchanged.

What the peel buys is the algebraicity of critical points: for $h$,
unlike $q$, the critical locus contains no curve.  That is the
keystone lemma below, and its proof is a pure characteristic-$2$
pleasantry.  Finally, the various nondegeneracy conditions accumulated
along the way ($W \neq 0$, $\Delta \neq 0$, $A \neq 0$, $B \neq 0$)
...
(Gauss's lemma) to a common factor in $\F_2[x,y]$, so they are
coprime, and a cleared B\'ezout identity gives
$su + tv = r(x) \neq 0$; evaluate at $(a,b)$ to get $r(a) = 0$.
Symmetrically for $b$.
\end{proof}

\begin{lemma}[keystone: no curve of critical points after the
peel]\label{lem:keystone}
...
By the kill lemma (Lemma~\ref{lem:kill}) applied to each of the
...
\emph{Engine.}  The chain rule for $h$, plus
$\partial_x h_x = 0$,
...
\emph{Fork, branch 1: $A$ and $B$ share a root $\theta$.}  Then
$P := (\alpha(\theta), \beta(\theta))$ is a common zero of
$h_x, h_y$, which by Lemmas~\ref{lem:keystone}
and~\ref{lem:algpoint} has algebraic coordinates.  Evaluate the peel
...
\begin{remark}[from $\F_2$ to the dichotomy]\label{rem:perfect}
...
cost, and no periodicity argument is needed.  For an arbitrary field
$k$ of characteristic $2$, embed $k$ into its perfection
$k^{1/2^\infty}$: a derivation of $x+y$ (or $xy$) over $k$ maps to
one over the perfection, contradicting the perfect case.  Finally,
for a ring $R$ in which $2$ is not a unit, $2$ lies in a maximal
ideal $\mathfrak m$, and a complete operation over $R$ would push
forward along $R \to R/\mathfrak m$ to a complete operation over a
field of characteristic $2$.  This proves
Theorem~\ref{thm:dichotomy}.
\end{remark}

Assembling: Theorem~\ref{thm:descent} $\Rightarrow$ (bridge,
Proposition~\ref{prop:bridge}) property $(\dagger)$ $\Rightarrow$
(master reduction, Proposition~\ref{prop:master})
...
\section{Aftermath: completeness when $2$ is invertible}
\label{sec:converse}

We now show characteristic $2$ is the only obstruction, by exhibiting
a complete operation everywhere else.  This section is independent of
Sections~\ref{sec:dichotomy}--\ref{sec:descent}.

\begin{theorem}\label{thm:converse}
Let $R$ be a commutative ring with $\tfrac12 \in R$.  Then
$q(x,y) := x^2 - y$ is complete: $\Clo(x^2 - y) = R[x,y]$.
\end{theorem}
...
\emph{Linear engine.}  For any $\alpha \in \Clo(q)$ and constant $k$,
...
\emph{Scaled squares.}  The same engine with $\alpha = y^2 + \ell$
...
even to span, and the derivation jams --- as, by Theorem~\ref{thm:Z},
every derivation must.
\end{remark}

\begin{remark}
Theorem~\ref{thm:converse} produces a complete operation of degree
$2$.  We do not know the complete picture for higher degrees; see
...
check.  All results stated here --- Theorems~\ref{thm:Z}, \ref{thm:F2}, \ref{thm:converse},
\ref{thm:dichotomy} and the supporting lemmas --- have
...
Appendix~\ref{app:lean}.  Two of the proofs presented above are
actually simplifications found during the verification process: the
derivative proof of the keystone (Lemma~\ref{lem:keystone}), which
replaced a longer argument via perfect function fields, and the
B\'ezout-certificate form of the kill lemma.  We will not say more
about the formalization here; the development is available from the
author.

\section{Open questions}\label{sec:open}

\begin{question}
Which degrees admit complete operations over $\Q$?  Degree $2$ does
...
engine appears to reach $xy$; we have not completed the
...
the clone of $x^3 - y$ consists only of polynomials of the form
...
into $x^3 - y$ and excluding $xy$; so $x^3 - y$ is certainly not
complete there.
\end{question}

\begin{question}\label{q:characterize}
Away from characteristic $2$, \emph{which} operations are complete?
Completeness is not simply ``nonlinear and depending on both
variables'': over $\Q$,
\begin{itemize}
\item $xy + 1$ is not complete, although it is nonlinear, uses both
  variables, and its clone contains $xy$.  The obstruction is a
  ...
  in particular $\alpha$ and $\beta$ --- lies in $\Q[x{+}y]$; the
  master reduction (whose proof is characteristic-free) then excludes
  $x+y$.
\item $x^2 + y$ is not complete, although $x^2 - y$ is: every element
...
whether completeness is governed by a finite explicit list of
...
$x^2 - 2y$, where the engine increment $\gamma \mapsto 4\gamma +
...
preliminary calculations suggest $1/x - y$ is complete over
...
Is membership in $\Clo(p)$ decidable?  Is completeness?
\end{question}
...
preserved by every substitution into $q$, however the intermediate
terms conspire --- membership in $\ker D$ in
...
cancellation conspires against, or a supply of such invariants
complete enough to certify every non-membership; we possess neither,
and we suspect the question is hard.  As a concrete instance of how
...
\medskip
\noindent\textbf{Draft note.}  The references below are reconstructed
from memory and must be verified (and surely extended) before this
goes anywhere; corrections welcome.
...
Completeness in Algebraic Systems}, Chapman \& Hall/CRC, 2001.
...
\bibitem{Pilz} G.~Pilz, \emph{Near-Rings}, 2nd ed., North-Holland,
1983.
...
Vollst\"andigkeit in den mehrwertigen Logiken}, Rozpravy
\v{C}eskoslovensk\'e Akad.\ V\v{e}d 80 (1970), 3--93.
...
wielowarto\'sciowych system\'ow logiki zda\'n}, C.~R.\ S\'eanc.\
Soc.\ Sci.\ Varsovie 32 (1939), 102--109.
...
For the record, the complete definitional trust base of the formal
verification ($R$ an arbitrary commutative ring; \texttt{X 0},
\texttt{X 1} the two variables of a bivariate polynomial ring;
\texttt{bind1 ![a,b] p} denotes the substitution $p(a,b)$):
