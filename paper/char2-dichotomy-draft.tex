...
Section~\ref{sec:precision} explains how this relates to --- and differs
...
Theorem~\ref{thm:Z} is the case $R = \Z$ of a complete dichotomy,
...
Half of this is inexpensive: over any commutative ring in which $2$
is invertible, the modest operation
\[
  p(x,y) \;=\; x^2 - y
\]
is complete --- a dozen lines of explicit algebra, presented in
...
Where should the negative half come from?  The first observation is
that completeness survives reduction of coefficients: a ring
...
where the obstruction actually lives, and not only over the prime
...
no field of characteristic $2$ admits a complete binary polynomial.
\end{theorem}

Finally, if $2$ is not a unit of $R$, it lies in a maximal ideal;
completeness would push forward along the quotient map to the
...
How does the formal question relate to the classical functional one?
...
be read in the function-clone world (on the infinite set $\Z$ or
$\Q$, where the classical finite-domain theory is silent).  Over
finite fields, however, the formal question is strictly finer than
the functional one, and the two can give \emph{opposite verdicts on
the same operation}:
...
\subsection{The rigidity property $(\dagger)$ and the master
reduction}\label{sec:tameness}
...
ingredients --- the parity decomposition, the master reduction, and
the bridge --- are unaffected, and the disjunctive conclusion follows
as over $\F_2$.
\end{proof}
...
If $2$ is a unit of $R$, the operation $x^2 - y$ is complete by
Theorem~\ref{thm:converse}.  If $2$ is not a unit, it lies in a
maximal ideal $\mathfrak m$, and a complete operation over $R$ would
push forward along $R \to R/\mathfrak m$ (Lemma~\ref{lem:transfer}
again) to a complete operation over the residue field, which has
characteristic $2$ --- contradicting Theorem~\ref{thm:char2}.
\end{proof}

\section{Relation to classical completeness theory}
\label{sec:context}
...
formal completeness does project onto a clean classical statement,
