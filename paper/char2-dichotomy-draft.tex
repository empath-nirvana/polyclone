...
$R \to S$ carries any derivation of $x+y$ over $R$ --- by a
\emph{derivation} we mean a formula building $x+y$ from the variables
and constants by repeated application of $p$ --- to one over $S$.
...
dense is most fragile, are precisely what the verification rules out.
...
In the latter direction, formal completeness does project onto a
clean classical statement, which we record for universal algebraists.
...
satisfy, and whether these interact with completeness of $p$, are
...
against the published record, with one exception: the issue and page
numbers of the Menger item could not be confirmed against the
original (the series and year are confirmed); it is marked in the
source.  Further pointers, especially to antecedents of the main
question, remain welcome.
